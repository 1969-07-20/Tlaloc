...
def print_qwerties(qwerties, qwerties_str):
    if 0 == len(qwerties.keys()):
        return

    print(qwerties_str)

#   if not all(qwerties[key]['count'] == 0 for key in qwerties.keys()):
#       print('')

    for key in sorted(qwerties.keys()):
        qwerty = qwerties[key]
        count = qwerty['count']
        ...
        with open(log, encoding='latin-1') as fp_in:
            ...
            for line in fp_in:
                ...
                if m0_result:
                    ...
                    if 1 == state:
                        print_errors(errors)
                        print_qwerties(qwerties, qwerties_str)
...
#                   print(f'yy:  {l_year}, {year}  mm:  {l_month}, {month}  dd:  {l_day}, {day}')
                    ...
                    if (beg_date <= log_date) and (log_date <= end_date):
                        ...
                        qwerties_str = ''
                        qwerties_str += ('\n')
                        qwerties_str += (f'-----------{"-" * len(log_timestamp)}-\n')
                        qwerties_str += (f'QUERIES ON {log_timestamp}:\n')
                        qwerties_str += (f'-----------{"-" * len(log_timestamp)}-\n')
#                       qwerties_str += ('\n')
                ...
                if 1 == state:
                    ...
                    if m1_result:
                        ...
                        qwerty_type = id_quote(q_url)

#                       if None is qwerty_type:
#                           print(f'UNKNOWN SRC  url={q_url}')
#                       else:
#                           print(f'SRC="{qwerty_type}"')

                        if qwerty_type in qwerties.keys():
                            qwerties[qwerty_type]['count'] += 1
                        else:
                            qwerties[qwerty_type] = {
                                'beg_time': time_stamp,
                                'end_time': time_stamp,
                                'count': 1,
                            }
