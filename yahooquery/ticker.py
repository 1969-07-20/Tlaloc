...
#   @property
    def technical_insights_LOC(self):
        """Technical Insights
    ...
    def all_financial_data_LOC(self, frequency="a"):
        """
        Retrieve all financial data, including income statement,
        balance sheet, cash flow, and valuation measures.
        ...
        return self._financials_LOC("cash_flow", frequency, types=types, trailing=False)
...
#   @property
    def corporate_events_LOC(self):
        return self._financials_LOC(
            "cash_flow", frequency=None, types=self.CORPORATE_EVENTS, trailing=False
        )
...
#   @property
    def corporate_guidance_LOC(self):
        """"""
        return self._financials_LOC(
            "cash_flow",
            frequency=None,
...
#   @property
    def valuation_measures_LOC(self):
        """Valuation Measures
        Retrieves valuation measures for most recent four quarters as well
        as the most recent date
        ...
        Only quarterly data is available for non-premium subscribers
        """
        return self._financials_LOC("valuation", "q")
    ...
    def balance_sheet_LOC(self, frequency="a", trailing=True):
        """Balance Sheet
        ...
        pandas.DataFrame
        """
        return self._financials_LOC("balance_sheet", frequency, trailing=trailing)
    ...
    def cash_flow_LOC(self, frequency="a", trailing=True):
        """Cash Flow
        ...
        pandas.DataFrame
        """
        return self._financials_LOC("cash_flow", frequency, trailing=trailing)
    ...
    def income_statement_LOC(self, frequency="a", trailing=True):
        """Income Statement
        ...
        pandas.DataFrame
        """
        return self._financials_LOC("income_statement", frequency, trailing=trailing)
    ...
    def history_LOC(
        ...
        params["interval"] = interval.lower()
        if params["interval"] == "1m" and period == "1mo":
            data = self._history_1m_LOC(adj_timezone, adj_ohlc)
        else:
            data = self._get_data("chart", params)
#           df = self._historical_data_to_dataframe(data, params, adj_timezone)
    ...
    def _history_1m_LOC(self, adj_timezone=True, adj_ohlc=False):
        params = {"interval": "1m"}
        today = datetime.today()
        dates = [
            convert_to_timestamp((today - timedelta(7 * x)).date()) for x in range(5)
        ...
        for i in range(len(dates) - 1):
            ...
            if merged_data is None:
                ...
                for symbol in data:
                    merged_data[symbol]['timestamp'] = data[symbol]['timestamp'] + merged_data[symbol]['timestamp']

                    merged_data[symbol]['indicators']['quote'][0]['open']   = data[symbol]['indicators']['quote'][0]['open']   + merged_data[symbol]['indicators']['quote'][0]['open']
                    merged_data[symbol]['indicators']['quote'][0]['high']   = data[symbol]['indicators']['quote'][0]['high']   + merged_data[symbol]['indicators']['quote'][0]['high']
                    merged_data[symbol]['indicators']['quote'][0]['low']    = data[symbol]['indicators']['quote'][0]['low']    + merged_data[symbol]['indicators']['quote'][0]['low']
                    merged_data[symbol]['indicators']['quote'][0]['close']  = data[symbol]['indicators']['quote'][0]['close']  + merged_data[symbol]['indicators']['quote'][0]['close']
                    merged_data[symbol]['indicators']['quote'][0]['volume'] = data[symbol]['indicators']['quote'][0]['volume'] + merged_data[symbol]['indicators']['quote'][0]['volume']

                    if 'adjclose' in data[symbol]['indicators']:
                        merged_data[symbol]['indicators']['adjclose'][0]['adjclose'] = merged_data[symbol]['indicators']['adjclose'][0]['adjclose'] + data[symbol]['indicators']['adjclose'][0]['adjclose']
...
#   @property
    def option_chain_LOC(self):
        ...
        for symbol in self._symbols:
            try:
                if data[symbol]["options"]:
                    merged_data[symbol] = data[symbol]["options"]
            except TypeError:
                pass

        if 0 < len(merged_data.keys):
           return merged_data
